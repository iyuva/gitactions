...
############# for_each used  ex 
# Create the EKS IAM role for each cluster
# resource "aws_iam_role" "eks_cluster_role" {
#   for_each = var.ekscluster # Assuming var.ekscluster is a map of clusters

#   name = "${each.value.cluster_name}-cluster-role" # Dynamically naming the role based on the cluster name
...
#   role       = aws_iam_role.eks_cluster_role[each.key].name # Access role by each.key
#   policy_arn = each.value
# }

# # Attach the Customer-Managed Policy (Allow and Deny) to the IAM Role
# resource "aws_iam_role_policy_attachment" "customer_managed_policy" {
#   for_each = var.ekscluster # Looping through each cluster
