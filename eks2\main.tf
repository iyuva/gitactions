...
resource "aws_eks_access_entry" "this" {
  ...
  cluster_name      = aws_eks_cluster.this[each.key].id
...
resource "aws_security_group" "cluster" {
  for_each = var.create && var.create_cluster_security_group ? { "cluster" = null } : {}
...
#   for_each = var.create && var.enable_irsa && !var.create_outposts_local_cluster && var.include_oidc_root_ca_thumbprint ? { "irsa" = null } : {}

#   url = aws_eks_cluster.this[0].identity[0].oidc[0].issuer
# }

# resource "aws_iam_openid_connect_provider" "oidc_provider" {
#   for_each = var.create && var.enable_irsa && !var.create_outposts_local_cluster ? { "oidc" = null } : {}
...
# Data resource for TLS Certificate using for_each
...
# IAM OpenID Connect Provider using for_each
resource "aws_iam_openid_connect_provider" "oidc_provider" {
  for_each = var.create && var.enable_irsa && !var.create_outposts_local_cluster ? { "oidc" = null } : {}

  client_id_list  = distinct(compact(concat(["sts.amazonaws.com"], var.openid_connect_audiences)))

  # Conditionally add thumbprints from the TLS certificate if it exists
  thumbprint_list = concat(
    var.include_oidc_root_ca_thumbprint && length(data.tls_certificate.this) > 0 ? [data.tls_certificate.this["irsa"].certificates[0].sha1_fingerprint] : [],
    var.custom_oidc_thumbprints
  )
